...
const VALID_TEACHER_TABS = ["today", "notes", "availability", "schedule", "messages"];
...
function setActiveTab(tabName) {
    ...
    if (tabName === "schedule") initScheduleTab();
}
    ...
    try {
        ...
        if (!data.lessons || data.lessons.length === 0) {
            ...
            data.lessons.forEach(l => {
                ...
                card.innerHTML = `
                    ...
                    <div class="lesson-time-display">${formatTimeRange(l.time, l.duration_min)}</div>
        ...
        if (data.cancelled && data.cancelled.length > 0) {
            ...
            data.cancelled.forEach(l => {
                ...
                card.innerHTML = `
                    <h3>${escapeHtml(l.student)}</h3>
                    <div class="lesson-time-display strikethrough">${formatTimeRange(l.time, l.duration_min)}</div>
...
async function openNotesModal(lessonId, studentName) {
    ...
    title.textContent = `Notes — ${studentName}`;
    ...
    try {
        ...
        subtitle.textContent = `${dateStr} · ${timeStr}`;
...
async function saveNotes(isAutosave = false) {
    ...
    if (!isAutosave) saveStatus.textContent = "Saving…";
    ...
    try {
        ...
        if (data.status === "success") {
            ...
            // Manual save (not autosave) → close modal after brief confirmation
            if (!isAutosave) {
...
async function sendNotesToStudent() {
    ...
    const saveStatus = document.getElementById("notes-save-status");
    sendBtn.disabled = true;
    saveStatus.textContent = "Sharing…";
...
// -------------------- LESSON NOTES TAB — students list --------------------
...
// -------------------- LESSON NOTES TAB — single-student history --------------------
...
async function openStudentHistory(studentId, studentName) {
    ...
    document.getElementById("student-history-subtitle").textContent = "Loading…";
    ...
    try {
        ...
        document.getElementById("student-history-subtitle").textContent =
            `${count} lesson${count === 1 ? "" : "s"}${data.student.voice_type ? ` · ${data.student.voice_type}` : ""}`;
        ...
        data.lessons.forEach(l => {
            ...
            if (!l.editable && !isCancelled) {
                badges.push(`<span class="lock-badge">Locked</span>`);
            }
            if (l.shared_with_student) {
                badges.push(`<span class="shared-badge">✓ Shared</span>`);
            }
            ...
            if (!isCancelled) {
                ...
                // Inline "Send to student" button — only if lesson has notes, isn't already shared, and isn't locked
...
async function submitRequest() {
    ...
    if (schedule.length === 0 && scope === "permanent") {
        const ok = confirm("You've left all days unticked — this means no availability at all. Save anyway?");
        if (!ok) return;
    }
...
// -------------------- SCHEDULE LESSONS (studio teachers) --------------------

let scheduleOrgStudents = [];
let scheduleTabInited = false;
let assignModalWeekday = null;
let assignModalTime = null;
let assignSelectedStudentId = null;
let scheduleDurationMin = 30;
let assignDurationMin = 30;

let _pillPopoverEl = null;
let _pillPopoverAnchor = null;

function _getPillPopover() {
    if (!_pillPopoverEl) {
        _pillPopoverEl = document.createElement("div");
        _pillPopoverEl.className = "schedule-pill-popover";
        _pillPopoverEl.style.display = "none";
        document.body.appendChild(_pillPopoverEl);
        document.addEventListener("click", (e) => {
            if (!_pillPopoverEl) return;
            if (_pillPopoverEl.contains(e.target)) return;
            if (_pillPopoverAnchor && _pillPopoverAnchor.contains(e.target)) return;
            hidePillPopover();
        }, true);
    }
    return _pillPopoverEl;
}

function showPillPopover(pill, assignment, weekday, slotTime, durationMin) {
    if (_pillPopoverAnchor === pill) { hidePillPopover(); return; }
    _pillPopoverAnchor = pill;

    const pop = _getPillPopover();
    const dayName = DAY_NAMES[weekday];
    pop.innerHTML = `
        <div class="spp-name">${escapeHtml(assignment.student_name)}</div>
        <div class="spp-meta">${dayName}s &middot; ${formatSlotTime(slotTime)} &middot; ${durationMin} min</div>
        <button type="button" class="spp-remove-btn">Remove assignment</button>
    `;
    pop.querySelector(".spp-remove-btn").addEventListener("click", () => {
        hidePillPopover();
        removeRecurringAssignment(assignment.id, weekday, slotTime);
    });
    ...
    requestAnimationFrame(() => {
        ...
        if (pr.left < 8) { left = r.left; transform = "translateY(-100%)"; }
        else if (pr.right > window.innerWidth - 8) { left = r.right; transform = "translateX(-100%) translateY(-100%)"; }
        if (pr.top < 60) {
...
function hidePillPopover() {
    if (_pillPopoverEl) _pillPopoverEl.style.display = "none";
    _pillPopoverAnchor = null;
}

function initScheduleTab() {
    if (scheduleTabInited) return;
    scheduleTabInited = true;

    // Duration chip picker
    document.querySelectorAll(".schedule-duration-chip").forEach(btn => {
        btn.addEventListener("click", () => {
            scheduleDurationMin = parseInt(btn.dataset.duration, 10);
            document.querySelectorAll(".schedule-duration-chip").forEach(b => b.classList.toggle("active", b === btn));
            loadScheduleWeekGrid();
        });
    });

    // Modal: student type toggle
    document.querySelectorAll('input[name="assign-student-type"]').forEach(r => {
        r.addEventListener("change", () => {
            const isExt = document.querySelector('input[name="assign-student-type"]:checked').value === "external";
            document.getElementById("assign-existing-student").classList.toggle("hidden", isExt);
            document.getElementById("assign-external-student").classList.toggle("hidden", !isExt);
        });
    });

    document.getElementById("assign-student-search")?.addEventListener("input", renderAssignStudentList);
    ...
    document.getElementById("assign-slot-modal")?.addEventListener("click", e => {
        if (e.target.id === "assign-slot-modal") closeAssignModal();
    });

    // Load org students eagerly so they're ready when the assign modal opens
    loadScheduleOrgStudents().then(() => renderAssignStudentList());
    loadScheduleWeekGrid();
}

async function loadScheduleOrgStudents() {
    try {
        const res = await fetch(`${API}/teacher/org-students`, { credentials: "include" });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        scheduleOrgStudents = Array.isArray(data) ? data : [];
    } catch (e) {
        console.error("loadScheduleOrgStudents:", e);
        scheduleOrgStudents = [];
    }
}

async function loadScheduleWeekGrid() {
    const grid = document.getElementById("schedule-week-grid");
    if (!grid) return;
    grid.innerHTML = `<em class="empty-note">Loading&hellip;</em>`;

    try {
        const res = await fetch(`${API}/teacher/weekly-schedule?duration=${scheduleDurationMin}`, { credentials: "include" });
        const days = await res.json();

        if (!days.length) {
            grid.innerHTML = `<em class="empty-note">No availability set. Add your schedule in the <strong>Schedule</strong> tab first.</em>`;
            return;
        }

        grid.innerHTML = "";
        days.forEach(day => {
            const assignedCount = day.slots.filter(s => s.assignment).length;
            const openCount = day.slots.length - assignedCount;
            const hasAssignments = assignedCount > 0;

            const details = document.createElement("details");
            details.className = "schedule-day-details";
            details.open = hasAssignments;

            const summary = document.createElement("summary");
            const badges = [
                assignedCount > 0 ? `<span class="schedule-day-count sdc-assigned">${assignedCount} assigned</span>` : "",
                openCount > 0     ? `<span class="schedule-day-count sdc-open">${openCount} open</span>` : "",
            ].join("");
            summary.innerHTML = `<span class="schedule-day-name-row">${escapeHtml(DAY_NAMES[day.weekday])}</span><span class="schedule-day-badges">${badges}</span>`;
            details.appendChild(summary);

            const slotsDiv = document.createElement("div");
            slotsDiv.className = "schedule-day-slots";
            ...
            day.slots.forEach(slot => {
                ...
                timeLabel.className = "schedule-slot-time";
                timeLabel.textContent = formatSlotTime(slot.time);
                row.appendChild(timeLabel);

                if (slot.assignment) {
                    const assignedDur = slot.assignment.duration_min || scheduleDurationMin;
                    const pill = document.createElement("div");
                    pill.className = "schedule-assigned-pill";
                    pill.innerHTML = `
                        <span>${escapeHtml(slot.assignment.student_name)}</span>
                        <span class="schedule-slot-dur">${assignedDur}m</span>
                        <button type="button" class="schedule-remove-btn" title="Remove recurring assignment">&times;</button>
                    `;
                    pill.querySelector(".schedule-remove-btn").addEventListener("click", (e) => {
                        e.stopPropagation();
                        removeRecurringAssignment(slot.assignment.id, day.weekday, slot.time);
                    });
                    pill.addEventListener("click", () => {
                        showPillPopover(pill, slot.assignment, day.weekday, slot.time, assignedDur);
                    });
                    row.appendChild(pill);
                } else {
                    const btn = document.createElement("button");
                    btn.type = "button";
                    btn.className = "slot-duration-chip schedule-assign-btn";
                    btn.textContent = "Assign";
                    btn.addEventListener("click", () => openAssignModal(day.weekday, slot.time, scheduleDurationMin));
                    row.appendChild(btn);
                }
        ...
        console.error(e);
        const grid2 = document.getElementById("schedule-week-grid");
        if (grid2) grid2.innerHTML = `<em class="empty-note">Failed to load schedule.</em>`;
    }
}

function openAssignModal(weekday, time, durationMin) {
    assignModalWeekday = weekday;
    assignModalTime = time;
    assignDurationMin = durationMin || scheduleDurationMin;
    assignSelectedStudentId = null;

    document.getElementById("assign-slot-title").textContent = `Assign Student — ${DAY_NAMES[weekday]}s at ${formatSlotTime(time)}`;
    document.getElementById("assign-slot-subtitle").textContent = `The next 12 sessions (${assignDurationMin} min each) will be booked automatically.`;
    document.getElementById("assign-modal-msg").textContent = "";

    // Reset form
    const existingRadio = document.querySelector('input[name="assign-student-type"][value="existing"]');
    if (existingRadio) existingRadio.checked = true;
    document.getElementById("assign-existing-student")?.classList.remove("hidden");
    document.getElementById("assign-external-student")?.classList.add("hidden");
    const search = document.getElementById("assign-student-search");
    if (search) search.value = "";
    const extName = document.getElementById("assign-ext-name");
    const extEmail = document.getElementById("assign-ext-email");
    if (extName) extName.value = "";
    if (extEmail) extEmail.value = "";

    renderAssignStudentList();
    document.getElementById("assign-slot-modal")?.classList.remove("hidden");
}

function closeAssignModal() {
    assignModalWeekday = null;
    assignModalTime = null;
    assignSelectedStudentId = null;
    document.getElementById("assign-slot-modal")?.classList.add("hidden");
}

function renderAssignStudentList() {
    const container = document.getElementById("assign-student-list");
    if (!container) return;
    const q = (document.getElementById("assign-student-search")?.value || "").toLowerCase();
    const filtered = q ? scheduleOrgStudents.filter(s => s.name.toLowerCase().includes(q)) : scheduleOrgStudents;
    container.innerHTML = "";
    if (!filtered.length) {
        const msg = scheduleOrgStudents.length === 0
            ? "No students in your org yet. Use the External option or invite students first."
            : "No students match your search.";
        container.innerHTML = `<em class='empty-note'>${msg}</em>`;
        return;
    }
    filtered.forEach(s => {
        const pill = document.createElement("span");
        pill.className = "dm-pill" + (assignSelectedStudentId === s.id ? " selected" : "");
        pill.textContent = s.name + (s.voice_type ? ` (${s.voice_type})` : "");
        pill.addEventListener("click", () => {
            assignSelectedStudentId = (assignSelectedStudentId === s.id) ? null : s.id;
            renderAssignStudentList();
        });
...
async function confirmAssign() {
    ...
    if (isExt) {
        external_name = (document.getElementById("assign-ext-name")?.value || "").trim();
        external_email = (document.getElementById("assign-ext-email")?.value || "").trim();
        if (!external_name) { msg.textContent = "Enter the student's name."; return; }
    } else {
        student_id = assignSelectedStudentId;
        if (!student_id) { msg.textContent = "Select a student from the list."; return; }
    }
    ...
    try {
        const res = await fetch(`${API}/teacher/recurring-lesson`, {
            method: "POST",
            credentials: "include",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                weekday: assignModalWeekday,
                time: assignModalTime,
                student_id,
                external_name,
                external_email,
                duration_min: assignDurationMin,
            })
        });
        const data = await res.json();
        if (data.status === "success") {
            ...
            msg.textContent = data.message || "Failed to assign.";
        }
    } catch (e) {
        console.error(e);
        msg.textContent = "Server error.";
...
async function removeRecurringAssignment(recurringId, weekday, time) {
    const ok = confirm(`Remove the recurring assignment for ${DAY_NAMES[weekday]}s at ${formatSlotTime(time)}? Existing booked lessons will not be cancelled.`);
    if (!ok) return;

    try {
        await fetch(`${API}/teacher/recurring-lesson/${recurringId}`, {
            method: "DELETE",
            credentials: "include",
        });
        loadScheduleWeekGrid();
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Reveal "Schedule Lessons" tab for studio teachers only
    if (ORG_TYPE === "studio") {
        document.getElementById("schedule-lessons-tab-btn")?.classList.remove("hidden");
    }
    ...
    document.addEventListener("visibilitychange", () => {
        ...
        loadWeekly();
        if (!document.getElementById("student-history-view").classList.contains("hidden")) {
            // Teacher was viewing a student's history — refresh that
            const view = document.getElementById("student-history-view");
            const sid = view.dataset.studentId;
            const sname = view.dataset.studentName;
            if (sid) openStudentHistory(Number(sid), sname);
        } else {
            // Teacher was on the students list — refresh that instead
            loadStudentsList();
        }
    });
    });
