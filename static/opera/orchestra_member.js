...
async function renderMyLessonToday(todayData) {
    ...
    try {
        ...
        if (!lessonToday) {
            ...
            const canCancel = canMemberCancelLesson(lessonToday, todayData.date, noticeMin);
            ...
            card.innerHTML = `
                ...
                ${!canCancel
                    ? `<p class="hint">Cancellation closes ${noticeStr} before the lesson.</p>`
                    : `<p class="hint">You can cancel up to ${noticeStr} before the lesson.</p>`}
            `;
...
function canMemberCancelLesson(lesson, bookableDate, noticeMin = 60) {
...
function buildOrchestraRehearsalCard(r, absences) {
    ...
    div.innerHTML = `
        ...
        <div class="rehearsal-card-footer">${actions}</div>
    `;
...
function renderBookingSection(data) {
    ...
    orchDurationOptions = (data.duration_options && data.duration_options.length) ? data.duration_options : [30];
...
let orchDurationOptions = [30];
let orchSlotDuration = 30;
let orchSlotTeacherId = null;
let orchSlotPeriod = null;
...
async function openSlotPicker(teacherId, teacherName, period) {
    ...
    orchSlotTeacherId = teacherId;
    orchSlotPeriod = period;
    orchSlotDuration = orchDurationOptions[0];
    ...
    renderOrchDurationChips();
    await loadOrchSlots();
}

function renderOrchDurationChips() {
    const grid = document.getElementById("slot-picker-grid");
    document.getElementById("slot-duration-chips")?.remove();
    if (orchDurationOptions.length <= 1) return;

    const bar = document.createElement("div");
    bar.id = "slot-duration-chips";
    bar.className = "slot-duration-bar";
    orchDurationOptions.forEach(d => {
        const btn = document.createElement("button");
        btn.type = "button";
        btn.className = "slot-duration-chip" + (d === orchSlotDuration ? " active" : "");
...
async function loadOrchSlots() {
    const grid = document.getElementById("slot-picker-grid");
    grid.innerHTML = `<em class="empty-note">Loading slots…</em>`;

    try {
        const res = await fetch(
            `${API}/orchestra-member/teacher-slots?teacher=${orchSlotTeacherId}&period=${orchSlotPeriod}&duration=${orchSlotDuration}`,
            { credentials: "include" }
        );
        ...
        slots.forEach(slot => {
            ...
            btn.addEventListener("click", () => bookOrchSlot(orchSlotTeacherId, slot));
...
async function bookOrchSlot(teacherId, time) {
    const msg = document.getElementById("slot-picker-msg");
    ...
    try {
        const res = await fetch(`${API}/orchestra-member/book`, {
            ...
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ teacher_id: teacherId, date: bookableDate, time, duration: orchSlotDuration })
        });
        const data = await res.json();
