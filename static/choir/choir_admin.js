...
const VALID_CHOIR_ADMIN_TABS = ["rehearsals", "subs", "sections", "ensemble", "messages"];
...
function setActiveTab(tabName) {
    if (!VALID_CHOIR_ADMIN_TABS.includes(tabName)) tabName = "rehearsals";
    ...
    if (tabName === "rehearsals") loadUpcoming();
...
function getTabFromURL() {
    ...
    return VALID_CHOIR_ADMIN_TABS.includes(hash) ? hash : "rehearsals";
}
...
async function renderUnifiedCaller(containerId, preCheckedSectionIds = [], preCheckedMemberIds = []) {
    ...
    if (type === "choir") {
        ...
        const SATB_ORDER = ["mezzo-soprano", "soprano", "mezzo", "alto", "contralto", "countertenor", "tenor", "baritone", "bass"];
        const satbKey = name => {
...
function openScheduleModal() {
    ...
    document.getElementById("reh-msg").className = "msg";
    document.querySelector("input[name='reh-mode'][value='single']").checked = true;
    document.querySelector("input[name='reh-choir-type'][value='choir']").checked = true;
    ...
    renderUnifiedCaller("unified-caller");
    document.getElementById("schedule-reh-modal").classList.remove("hidden");
}
...
async function createRehearsal() {
    ...
    try {
        ...
        if (data.status === "success") {
            ...
            setTimeout(() => {
                document.getElementById("schedule-reh-modal").classList.add("hidden");
                loadUpcoming();
            }, 800);
        } else {
            msg.className = "msg";
...
async function openFindSubModal(rehearsalId, sectionId, sectionName) {
    ...
    try {
        ...
        if (preferred.length) {
            ...
            contactBtn.className = "slot-pill-btn";
            contactBtn.style.cssText = "width:100%;margin-bottom:var(--space-4);font-size:.88rem;";
            contactBtn.textContent = "Contact Preferred Subs";
            contactBtn.addEventListener("click", () => adminContactAllPreferred(findSubRehearsalId, findSubSectionId, contactBtn));
...
async function adminContactAllPreferred(rehearsalId, sectionId, btn) {
    btn.disabled = true;
    btn.textContent = "Sending...";
    try {
        const res = await fetch(`${API}/choir/contact-preferred-subs`, {
            method: "POST", credentials: "include",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ rehearsal_id: Number(rehearsalId), section_id: Number(sectionId) }),
        });
        const data = await res.json();
        if (data.status === "success") {
...
function buildSubRow(s) {
    ...
    const rankHtml = s.is_preferred
        ? `<label style="display:flex;align-items:center;gap:4px;font-size:.8rem;color:var(--text-muted);white-space:nowrap;">
               #<input type="number" class="rank-input" min="1" value="${s.preferred_rank != null ? s.preferred_rank : ''}"
                   placeholder="—" title="Contact rank (1 = first)"
                   style="width:3.2rem;text-align:center;padding:2px 4px;font-size:.85rem;">
           </label>`
        : "";
    ...
    if (s.is_preferred) {
        ...
        rankInput.addEventListener("change", async () => {
            ...
            if (raw !== "" && (isNaN(rank) || rank < 1)) {
                rankInput.value = s.preferred_rank != null ? s.preferred_rank : "";
                return;
            }
...
async function loadInvitations() {
    ...
    list.innerHTML = `<em class="empty-note">Loading…</em>`;
    const roleFilter = document.querySelector("input[name='invite-member-type']:checked")?.value;
    try {
        const res = await fetch(`${API}/admin/invitations`, { credentials: "include" });
        const data = await res.json();
        const all = Array.isArray(data) ? data : (data.invitations || []);
        const invites = roleFilter ? all.filter(i => i.role === roleFilter) : all;
        if (!invites.length) {
...
async function loadEnsembleMembers() {
    ...
    try {
        ...
        if (!members.length) {
            list.innerHTML = `<em class="empty-note">No ensemble members yet. Use the Add Member button to invite them.</em>`;
            return;
        }
        list.innerHTML = "";
        members.forEach(m => {
...
function openInviteModal(defaultRole = "choir_member") {
    document.getElementById("invite-email").value = "";
    document.getElementById("invite-name").value = "";
    document.getElementById("invite-instrument").value = "";
    document.getElementById("invite-msg").textContent = "";
    document.getElementById("invite-msg").className = "msg";
    const radio = document.querySelector(`input[name="invite-member-type"][value="${defaultRole}"]`);
    if (radio) radio.checked = true;
    document.getElementById("invite-instrument-row").classList.toggle("hidden", defaultRole !== "ensemble_member");
    loadInvitations();
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Invite modal — member type toggle shows/hides instrument field and re-filters list
    ...
    // Add Member buttons
    document.getElementById("add-choir-member-btn").addEventListener("click", () => openInviteModal("choir_member"));
    document.getElementById("add-ensemble-member-btn").addEventListener("click", () => openInviteModal("ensemble_member"));
    ...
    // Rehearsals tab — open/close schedule modal
    ...
    document.getElementById("close-schedule-modal-btn").addEventListener("click", () =>
        document.getElementById("schedule-reh-modal").classList.add("hidden"));
    document.getElementById("schedule-reh-modal").addEventListener("click", e => {
        if (e.target.id === "schedule-reh-modal") e.target.classList.add("hidden");
    });

    // Schedule modal — submit (delegates based on mode)
    ...
    // Schedule modal — bulk preview updates
