...
let orchestraTodayRehearsals = [];

function openOrchestraViewNotes(rehearsalId) {
    const r = orchestraTodayRehearsals.find(x => x.id === rehearsalId);
    if (!r) return;
...
function renderRehearsals(data) {
    ...
    orchestraTodayRehearsals = data.rehearsals;
    ...
    box.querySelectorAll(".view-reh-notes-btn").forEach(btn => {
        btn.addEventListener("click", () => openOrchestraViewNotes(Number(btn.dataset.id)));
    });
}
